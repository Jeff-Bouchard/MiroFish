...
使用内置管理脚本，一次性填写必需配置并管理 Docker 部署：
...
# 启动带 Branding 页签的可视化表单（用于 whitelabeling）
...
- `LLM_API_KEY`（必填）
- `ZEP_API_KEY`（必填）
- `LLM_BASE_URL`、`LLM_MODEL_NAME`（推荐）
- 可选 `LLM_BOOST_*` 配置
- 前后端宿主机端口（自动校验端口可用性）
...
- 自动枚举所有图片资产与品牌元素
- 展示尺寸（宽×高）、长宽比、文件体积、引用位置
- 先加入白名单，再通过浏览按钮上传替换文件
- 上传时执行文件类型、体积上限和长宽比兼容校验（便于公开场景）

Text Branding 页签支持：

- 自动发现品牌文本 token（如 `MIROFISH`、slogan 等）
- 展示 token 所在文件、行号、出现次数
- 先加入白名单，再执行文本替换
- 替换前对目标文件做备份，便于回滚
